...
import sys
# Get project root (2 levels up from this file: layer0 -> src -> root)
...
def load_config(config_file:str = None) -> dict:
    '''
    config_file: input config file path (if None, tries new location then legacy)
    return: a dict of configuration
    '''
    # Determine config file path with fallback
    if config_file is None:
        new_path = PROJECT_ROOT / 'research_config' / 'research_domain.yaml'
        legacy_path = PROJECT_ROOT / 'config.yaml'

        if new_path.exists():
            config_file = str(new_path)
            logging.info(f"Using new config location: {config_file}")
        elif legacy_path.exists():
            config_file = str(legacy_path)
            logging.warning(f"Using legacy config location: {config_file} (consider migrating to research_config/)")
        else:
            raise FileNotFoundError("No config file found at research_config/research_domain.yaml or config.yaml")
    ...
    def pretty_filters(**config) -> dict:
        ...
        keywords_key = 'keywords' if 'keywords' in config else 'categories'
        for k,v in config.get(keywords_key, {}).items():
            if 'filters' in v:
...
def _load_researcher_profile_sections():
    ...
    """
    try:
        with open(str(RESEARCHER_PROFILE_PATH), "r", encoding="utf-8") as f:
            content = f.read()
    except FileNotFoundError:
