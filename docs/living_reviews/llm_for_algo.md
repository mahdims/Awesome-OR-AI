...
### Front 4 (23 papers) — STABLE

**Density:** 0.29 | **Methods:** llm_code_generation, program_synthesis, llm_evolutionary_search, evolution_of_heuristics, llm_as_heuristic | **Problems:** algorithm_discovery, circle_packing, heuristic_evolution, automated_algorithm_design, autocorrelation_inequalities
...
This research front centers on the AlphaEvolve framework and its numerous extensions, open-source reproductions (GigaEvo, CodeEvolve), and direct competitors (ShinkaEvolve, ThetaEvolve, DeltaEvolve, ImprovEvolve, LoongFlow, DGM, AlphaResearch, SATLUTION, READY, SS-Logic). The unifying theme is the LLM-guided evolutionary search for discovering and optimizing executable programs and algorithms, ranging from compiler heuristics and mathematical constructions to novel activation functions and self-improving agents. The focus is on pushing the boundaries of automated algorithm design by evolving code directly, often in Python or C++.

Key contributions include significant advancements in sample efficiency and generalization. ShinkaEvolve achieved new SOTA on circle packing with <150 samples, while DeltaEvolve reduced token consumption by ~37% by using 'semantic deltas'. Magellan introduced a 'Hierarchical Search' for compiler heuristics, achieving >5% binary size reduction in LLVM. SATLUTION extended LLM evolution to full C++ repositories, outperforming 2025 human SAT competition winners. ImprovEvolve demonstrated new SOTA on Hexagon Packing by evolving modular local search operators. ThetaEvolve and TTT-Discover integrated test-time reinforcement learning, setting new bounds on Circle Packing and Autocorrelation inequalities. READY achieved superior performance over Eureka and EoH on BBOB benchmarks for reward discovery, with a 2-4x reduction in search time. C-Evolve introduced an EMA voting score for evolving complementary prompt portfolios, beating AlphaEvolve by ~4% on Qwen3-8B. Berthold et al. provided a crucial reality check, showing that standard global NLP solvers can outperform AlphaEvolve on its own benchmarks.

This research front is rapidly maturing, with a strong emphasis on addressing the practical limitations of LLM-driven evolutionary search, particularly sample efficiency, generalization, and scalability to complex, multi-file codebases. The next wave of papers will likely focus on integrating more sophisticated meta-learning loops, such as self-evolving rule systems (SATLUTION) and lineage-based context retrieval (LoongFlow), to enable truly open-ended and autonomous algorithm discovery. There will also be continued efforts to develop robust benchmarking methodologies that account for classical solver performance and real-world deployment challenges, moving beyond isolated kernel optimization towards full-system evolution.
...
**Density:** 0.50 | **Methods:** llm_code_generation, llm_as_heuristic, program_synthesis, funsearch, evolution_of_heuristics | **Problems:** heuristic_evolution, operator_discovery, algorithm_discovery, combinatorial_optimization, hyperparameter_optimization
...
**Density:** 0.76 | **Methods:** program_synthesis, llm_evolutionary_search, llm_as_heuristic, llm_code_generation, evolution_of_heuristics | **Problems:** heuristic_evolution, tsp, bin_packing, operator_discovery, traveling_salesman_problem
...
### Front 6 (11 papers) — STABLE

**Density:** 0.49 | **Methods:** llm_code_generation, program_synthesis, llm_as_heuristic, evolution_of_heuristics, llm_evolutionary_search | **Problems:** tsp, combinatorial_optimization, heuristic_evolution, bin_packing, cvrp
...
### Front 3 (6 papers) — STABLE

**Density:** 0.47 | **Methods:** llm_code_generation, llm_evolutionary_search, evolutionary_algorithm, llamea, genetic_programming | **Problems:** black_box_optimization, heuristic_evolution, automated_algorithm_design, expensive_continuous_optimization, operator_discovery
...
**TRUE SYNTHESIS** | score=0.68 | Front 6 → Front 0, Front 2
...
**TRUE SYNTHESIS** | score=0.67 | Front 3 → Front 0
...
**TRUE SYNTHESIS** | score=0.65 | Front 3 → Front 2, Front 0
...
**TRUE SYNTHESIS** | score=0.63 | Front 6 → Front 0, Front 4, Front 2, Front 3
